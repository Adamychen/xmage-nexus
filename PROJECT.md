...
> Last updated: 2026-08-25 (contract anti-drift closed: `types.generated.ts` regenerado para casar con `contract.schema.json` — conserva `CardView.sourceCard` y `UsersView.avatarId`, elimina 11 campos muertos; CI valida `gen-zod:validate` + `gen-types:validate`; Plan A: `gameViewSchema` + centralización `deck-names`/`table-names`/`FAKE_MODE` commiteado — 9e91197a04 y 4a2e8aac48)
