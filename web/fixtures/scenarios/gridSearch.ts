...
export function gridSearchScenario(): Scenario {
  ...
  for (let i = 0; i < 11; i++) choices[`mode-${i}`] = `Battle tactic ${i}`
  choices['mode-dragon'] = 'Summon Ancient Dragon'

  const emitChoice = () => {
    activeConn?.broadcast('GAME_CHOOSE_MODE', {
      message: 'Choose a battle tactic',
      choices,
  ...
  return makeBaseScenario({
    tableId,
    tableName: TABLE.gridSearch ?? 'GRID_SEARCH',
    gameId,
    getGameView,
    onConnect: track,
