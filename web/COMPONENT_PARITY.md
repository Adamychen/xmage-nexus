...
|---|---|---|---|---|---|---|
| U1 | Shell / Conexión | `mage/client/MageFrame.java`, `MagePane.java`, `dialog/ConnectDialog.java`, `RegisterUserDialog.java`, `ResetPasswordDialog.java` | `src/App.tsx`, `lobby/LoginScreen.tsx` | ✅ | Login supera al desktop (presets, split proxy/servidor, avatar, i18n). Registro/reset **no aplican**: el servidor lleva `authenticationActivated=false` por defecto (`config.xml:58`, *"user need not to register"*); `Session.registerUser` responde `REGISTRATION_DISABLED_MESSAGE` y el login ignora la pass (`Session.java:88-93,246-255`; `Main.java:87` "no password check"). Solo tendrían sentido contra un servidor con auth activada. **2026-09-16**: por eso mismo, el campo Contraseña se deshabilita con aviso cuando el destino es `beta.xmage.today` (LoginScreen y SetupWizard: `passwordDisabledOnBeta`, se limpia y nunca viaja en la conexión). **2026-09-20**: ese disable aplica solo a la contraseña de **cuenta**; la de **mesa** no depende de `authenticationActivated` y sigue operativa en beta (ver U3/U4). | 2026-09-20 |
| U2 | Lobby mesas | `table/TablesPanel.java`, `TablesPane.java`, `TablesTableModel.java`, `MatchesTableModel.java` | `lobby/LobbyScreen.tsx`, `TableCard.tsx`, `TableFilterBar.tsx`, `FinishedMatchesPanel.tsx` | ✅ | Filtros desktop (rated/unrated, abierta/privada, torneo constructed/limited, ocultar ignorados con toggle OFF por defecto + aviso en join) + orden desktop-default (libres primero, recientes) con selector + búsqueda propia por texto (el desktop no la tiene) + PM/whisper/ignore + doble-clic unirse/espectar (`TableFilterBar.test.tsx` 13 tests). **En vivo 2026-09-12 (plan §§10–11)**: lobby Bo3 ("Wins: 2 Score: 0-0/0-1", pips en PlayerInfoBar) con re-espectado por partida (sin auto-follow = paridad desktop); vías de error honestas (mesa llena `[SEAT]`, password mala `[PASSWORD]`); fin de partida de espectador 100% sintético (`END_GAME_INFO` no llega a watchers — se conserva Volver-al-lobby). **Tests (sin vivo)**: deep-links `#join=`/`#watch=` — los 2 e2e `invite-link.spec` pasan (known-broken vacío desde 2026-09-12; era coletazo del SetupWizard), `useInviteLink` cubierto por tests. **B.10 follow Bo3 en vivo 2026-09-14** (mesa `b10-follow`, 2×HUMAN): aviso «La partida cambió» + `Seguir partida` → board de la 2ª/3ª; destapó un bug de raíz (el `GAME_INIT` de la partida nueva se descartaba como stale contra la vista anterior → staging congelado) arreglado en `handleWatchGame`+`eventHandler` con 2 tests rojos→verdes | 2026-09-14 |
| U3 | Crear mesa | `dialog/NewTableDialog.java`, `table/TablePlayerPanel.java`, `NewPlayerPanel.java` | `lobby/CreateTableDialog.tsx` + `lobby/CreateTable/` | ✅ | Paridad cerrada 2026-09-06 (delta sobre §F): skill por plaza 1-10 default 2 (`seatSkills` web→`SimManager`→`SimPlayer` + skill propia) como `NewPlayerPanel.spnLevel`, bannedUsers con UI (proxy ya lo parseaba), numberRounds con UI (0=auto), range/attack por flags del servidor (`GameTypeView.useRange/useAttackOption`, fallback heurística), validación por paso (nombre+compat+ocupantes), hint vivo por plaza (cartas del mazo + aviso vacío). F11 2026-09-06: plazas HUMAN en espera (`Humano — espera rival` por plaza + chip global, como `TablePlayerPanel=HUMAN`; tipos normalizados a enum canónico). Fuera de alcance: F6 emblemas `.dck` (experimental desktop, 3 capas). **2026-09-16**: en `beta.xmage.today` el campo Contraseña (pestaña Restricciones) se deshabilita con aviso y nunca se envía (mismo criterio que U4-2 en JoinTableDialog; `useCreateTableForm.passwordDisabledOnBeta` + i18n `create_password_disabled_beta` ×9). **2026-09-20 (revertido para mesas)**: premisa falsa — el motor comprueba la password de mesa siempre (`TableController.joinTable`, `Wrong password.`), `authenticationActivated=false` solo afecta a la contraseña de cuenta; el campo vuelve a estar operativo en beta y la password viaja en `createTable`/`joinTable` (verificado en vivo: mesa privada real creada con `passworded:true`). Tests: `CreateTableDialog.test` 8/8 (incluye campo activo en beta + password en payload) + `constants.test` 4/4 + `wizard.spec.ts` 3/3 fake con aserción del frame WS | 2026-09-20 |
| U4 | Unirse / Espera / Staging | `dialog/JoinTableDialog.java`, `TableWaitingDialog.java` | `lobby/JoinTableDialog.tsx`, `SpectatorStagingScreen.tsx` | ✅ | Auditada 2026-09-06 (ver § U4): swapSeats (proxy+↑/↓ dueño en READY), torneo (bypass join sin mazo en limitado + `startTournament`), roster rico (rating limited/constructed + history + flag), start con confirm si falta ready (aviso, no bloqueo). 2026-09-07: chat por mesa U4-11 cerrado (proxy `getTableChatId` + `joinChat/leaveChat` por ciclo de sala + `ChatBox chatIdOverride` + marcas Listo a la mesa). Stretch restante: ninguno (U4-2 y U4-12 cerrados 2026-09-14; **2026-09-20**: la contraseña de mesa vuelve a estar operativa en `beta.xmage.today` — el disable de U4-2 era una generalización errónea desde la contraseña de cuenta; verificado en vivo: join con password incorrecta rechazado y con la correcta aceptado, diálogo de unión con campo activo). Tests: `TableStagingCommandsTest` 7/7 java + `store.test` 7 nuevos + `staging.spec.ts` 8/8 + `staging-chat.spec.ts` 2/2 fake + verificación real 11/11 (2 sesiones + espectador + aislamiento entre mesas). **En vivo 2026-09-12 (plan §10)**: join con contraseña + staging privada + leave/limpieza re-verificados (Fase 2B-rest); Bo3 `bo3-1` con `SIDEBOARD` entre partidas (submits por WS crudo — gap de harness, sin `submitDeck` en MCP —, bibliotecas restauradas a 55); `SideboardScreen`/`GameEndDialog` cableados con tests. **En vivo 2026-09-18 (p6-C1, `docs/history/qa/p6-live-validations.md`)**: submit de `SideboardScreen` **por UI** en Bo3 real HUMAN+SIM — partida 1 ganada (11 veneno), la web abre el banquillo, movimiento Bosque banquillo→principal (DOM `Mazo Principal 61` / `Banquillo 14/15`) y «Enviar mazo»; la partida 2 arranca con **biblioteca 54** (=61−7; 53 si el submit no hubiera llegado) → el submit llegó al servidor. Hallazgo F1: el tablero de la web se quedó congelado en la partida 1 hasta recargar (ver U11/C4) | 2026-09-20 |
| U5 | Chat | `chat/ChatPanelBasic.java`, `ChatPanelSeparated.java`, `table/PlayersChatPanel.java` | `game/GameChat.tsx`, `lobby/ChatBox.tsx` | ✅ | Auditada 2026-09-07: envío/recepción sala+mesa+partida ✅, whisper `/w` (envío por parser del servidor + `WHISPER_FROM/TO` diferenciados con sonido) ✅, `/history`/`/me`/`/card`/`/list` (responden `USER_INFO` → sistema) ✅, `[Carta]` + hover ✅ (supera), split Talk/System ≈ toggle + canales ✅, usuarios (flag/rank/ping) + `UserActionModal` ✅, ventana separada ≈ `FloatingChat` arrastrable ✅. Delta: hora tenue por mensaje (U5-2, `time` del evento), propio en verde (U5-4), límite 500 con aviso i18n (U5-3, `GameChat` ya lo tenía). Diferencias declaradas: ignore solo-cliente (el desktop bloquea joins; cubrir con password/vetados), profanity fuera de alcance. Tests: `ChatBox.test` +3 + `chat.spec`/`staging-chat.spec` asserts de hora + verificación real 8/8 (whisper A↔B, `/history`, `/card`, >500 lo acepta el servidor). **En vivo 2026-09-12 (plan §11, `audit-2c3`)**: chat de partida como espectador (mensaje con hora + autor). **Deuda abierta (plan2 B.9)**: el chat flotante tapa botones en Mazos (z-order `FloatingChat` z-60 vs contenido; repro plan §4) — pendiente de fix. **C3 2026-09-18 (`docs/history/qa/p6-live-validations.md`)**: `UserActionModal` = modal de usuario del lobby (susurro/perfil/ignorar), renderizado sólo en `LobbyScreen.tsx:381` y disparado desde chat/listado del lobby; `GameChat` no tiene `onUserClick` → **mismo flujo que U5, sin camino de juego distinto** (re-verificado en vivo con 2ª sesión + chat global) | 2026-09-18 |
| U6 | Editor mazos | `deckeditor/DeckEditorPanel.java`, `CardSelector.java`, `DeckArea.java`, `DeckLegalityPanel.java`, `collection/viewer/` | `decks/DeckBuilder.tsx` + `decks/*` (69 ficheros) | ✅ | Auditada 2026-09-07 (ver § U6): sort resultados (6 órdenes + dirección, U6-1), Mana Analyser completo (fuentes/básicas/distribución, U6-2), +4 formatos (Oathbreaker, PD Commander, Highlander EU/CA, U6-3), import `.dek`/`.cod`/`.o8d` + export `.dek` (U6-4/5), slider tamaño carta. Supera: sintaxis Scryfall, sample hand London + goldfish, catálogo meta + Moxfield/Archidekt, reparación de issues del servidor. No-aplica: `CollectionViewer/MageBook` (sin DB local), Open Booster, Bling, Draft Rating, LAYOUT posicional. Resto import/export (`.mwdeck/.draft/.json`) → U7. Tests: unit 883 + `decks-gallery.spec` 5/5 (nuevo test U6) + `deckvalidation` 3/3 | **UX pass 2026-09-14** (auditoría, sin mecánicas nuevas): el guardado pendiente ya no se pierde al cerrar (flush del debounce + limpieza); rejilla y tira operables por teclado (Enter/Espacio, `focus-within`, sin que los botones burbujeen a la tira); hint de papelera honesto («quitar una copia»); etiquetas claras (Cargar más/Añadir, Guardando/Guardado, Equipar/Equipado por id, pips de maná, tierras objetivo); errores de legalidad visibles al clic (popover) y búsqueda con estados (sin resultados/error+reintento); banquillo con tope por formato; curva con vacío y aria; confirmaciones al borrar copias y al sugerir tierras; preview clampada. Tests: `DeckBuilder.test` (persistencia al cerrar), `ArenaCardStrip.test` (teclado/confirmación/foco), `ArenaCardGrid.test` (teclado/no-resultados/reintento/contador), unit 1500. **Mana pies 2026-09-14**: los repartos `conic-gradient` de 38px se sustituyen por `ManaPie` (SVG con hueco de 2px entre sectores, total central y leyenda nombre/cantidad/%; `ManaPie.test` 6 casos; `COLOR_LABEL_KEYS` exportado desde `ArenaFilterBar`), unit 1506. | 2026-09-14 |
...
| U8 | Generador mazos | `deck/generator/DeckGenerator*.java` (5), `RatioAdjustingSliderPanel.java` | — (sin equivalente en `web/src`) | ➖ | Fuera de alcance por decisión 2026-09-14 | 2026-09-14 |
| U9 | Draft | `draft/DraftPanel.java`, `DraftGrid.java` | `game/DraftScreen.tsx` | ✅ | Auditada 2026-09-08 (ver § U9): ocultar pickeadas (Hide+F9, `hiddenCards` en el pick — el proxy ya lo soportaba) U9-1, confirm quit U9-2, mesa/asientos + dirección de paso (usa `players` antes ignorado) U9-3, timer naranja ≤30s + tick audio 6s + aviso con pestaña oculta U9-4, anti-doble-pick 1.5s U9-5, descarga log `.draft` reimportable por U7-1 U9-6, sobre ordenado por rareza como desktop U9-7. Supera: hover doble-cara, imágenes Scryfall, i18n. Fix colateral: `DraftScreen`/`ConstructScreen` montados 2× (`App`+`GameScreen`) → solo `App`. Tests: unit 903 + `draft.spec` 3/3. **En vivo 2026-09-13 (plan §12, `audit-sealed-2`)**: ciclo de torneo Sellado completo contra protocolo real (create/join/start + `joinTournament` ×2 — sin esto no arranca — → `CONSTRUCT` pool 28KB M20 → auto-submit → partida 40 cartas → starting player → keeps → T1–T2 → concesión → Finished). **Límite declarado (corregido 2026-09-14)**: A.1 resuelto hoy (picks de draft con la web, local y beta); A.2 (construcción sealed vía UI, `ConstructScreen` submit 40) ya estaba verificado en vivo el 2026-09-14 con sealed desde la UI (pool 90 + sugerida de tierras + submit 40, plan3 §1.2); la nota anterior («pendiente, se usó auto-submit») era de la primera prueba de auditoría y quedó obsoleta. **2026-09-14 (fix UX + vivo)**: el cliente ignoraba la respuesta de `sendCardPick` (el proxy envía el `DraftPickView` con `picking:false`) y no había evento del server hasta que TODOS pickeaban → la UI seguía en «Tu turno», el contador corría a 0 («tiempo agotado») y las cartas seguían clicables; sin confirmación de qué carta elegiste. Ahora: acuse inmediato (banner «Has elegido {carta}» + chip «Esperando…», sin contador), `mergePickAck` sincroniza sobre/picks con guarda anti-respuesta-obsoleta, `DRAFT_UPDATE` fuerza `picking:false` (autopicks sin respuesta), anti-doble-pick por acuse, recarga en mitad del pick habilitada (DRAFT_INIT), progreso «Sobre X/N · Carta Y», texto de dirección de paso, highlight del pick nuevo y nombre resuelto por Scryfall cuando `SimpleCardView` no trae `name` (protocolo real). Tests: `draft.test` (+5), `DraftScreen.test` (+5), e2e `draft.spec` +1 (fake con silencio real + respuesta DraftPickView); unit 1516. **Vivo 2026-09-14**: draft real 2×HUMAN (`Booster Draft Elimination`, M21x1 vía WS crudo de control) — espera larga verificada (captura `draft-live-waiting.png`), rival pickea → siguiente pick re-habilita solo (`draft-live-resumed.png`); límite A.1 resuelto. **También en beta.xmage.today** (2026-09-14): misma mesa 2×HUMAN contra el server oficial (login anónimo OK, 98 online), nombres reales por Scryfall, espera y re-habilitación (`beta-draft-waiting.png`/`beta-draft-resumed.png`). **Resync tras recarga 2026-09-14**: el server no reenvía `DRAFT_INIT` a un `joinDraft` tardío, así que la web persiste la instantánea completa del draft (máx 3h) y al reconectar la re-pinta + re-une la sesión (limpieza en over/construct/quit/login/reset); vivo 2×HUMAN: retorno inmediato a la pantalla y pick aceptado desde la restaurada (`draft-live3-resync.png`) | 2026-09-14 |
| U10 | Torneo | `tournament/TournamentPanel.java`, `dialog/NewTournamentDialog.java`, `RandomPacksSelectorDialog.java` | `game/TournamentPanel.tsx`, `lobby/TournamentBracket.tsx`, `TournamentStandings.tsx` | ✅ (AUDITADA 2026-09-08, CERRADA; ver §U10) | `TOURNAMENT_*` ✅ (`tournament.spec.ts` 4/4: bracket/modal, T1 watch, T4 chat, T5 packs; Fase A `849ec37f20` + Fase B). **En vivo 2026-09-12/13**: wizard Torneo con datos reales (19 tipos Draft/Sealed/Jumpstart, sets, plazas 2–32, sobres 3/6, tiempos; plan §2 + §12, screenshot `page-2026-09-13T08-35-10`) + torneo Sealed Elimination `audit-sealed-2` de punta a punta (Finished, A Winner 3pts, ronda "Player qa-seal-A is the winner" vía `getTournament`). **Límites declarados**: picks con la web ✅ resueltos 2026-09-14 (draft real 2×HUMAN, ver U9); submit `ConstructScreen` vía UI ✅ (sealed, 2026-09-14, ver U9); staging de torneo + bracket con torneo vivo de 2 pendiente (plan2 A.4); espectador sin auto-follow Bo3/torneo = paridad desktop (plan §10). **En vivo 2026-09-14 (`perf-sealed-1`)**: espectar un match de torneo **en vivo** desde el lobby no es posible hoy — «Ver Cuadro» solo lista partidas finalizadas y «Espectar» deja la pantalla de staging («Preparando inicio…») sin seguir el match; el watch real exige `watchTournamentTable(matchTableId)` (sin UI fuera del `TournamentPanel` del participante; el proxy/MCP tampoco lo exponen). **2026-09-14 (3ª parte)**: `getTournament` de la final da `Finished`/winner/3pts, pero la mesa desaparece del lobby al terminar; y `Ver Cuadro` de una mesa AJENA pintaba el torneo propio cacheado (bug) → fix `isOwnTournament` en `useTournamentBracket.ts` + 2 tests rojos→verdes. **2026-09-14 (5ª parte)**: watch de match de torneo EN VIVO desde el lobby RESUELTO — `SHOW_TOURNAMENT` trae el tournamentId real (el server no resuelve ids de mesa en `getTournament`), el lobby lo consume y abre el cuadro con el id real; «Espectar» de la tarjeta abre el cuadro (sin staging) y el ojo del cuadro pinta el match como espectador (verificado en vivo, torneo Constructed con match Dueling); en torneos ajenos ya no aparece «Abandonar torneo» (usa el id real solo si es propio). **C5 2026-09-18 (`docs/history/qa/p6-live-validations.md`)**: torneo real de 2 (`Constructed Elimination`, Constructed-Pioneer, 2×HUMAN) — staging 2/2 Listos en la web, arranque + `joinTournament` ×2, bracket en vivo con Ronda 1 «EN PARTIDA», standings «En partida» y ojo de watch; quit por UI → FINALIZADA (c5b Winner 3pts). Nota harness MCP F2: el default `tournamentType:"Elimination"` da NPE en el server; el nombre real es `Constructed Elimination` | 2026-09-18 |
| U11 | Núcleo partida | `game/GamePanel.java`, `GamePane.java` | `game/GameScreen.tsx`, `state/eventHandler.ts`, `state/events/` | ✅ | Auditada 2026-09-05, **cerrada 2026-09-08**: G11-1 skips, G11-2 trigger-order, G11-3 auto-answers; G11-4 macros declarado no-aplica (botón vestigial sin consumidor en motor/servidor); G11-5 paradas de fase con defaults persistentes todo-marcados. **En vivo 2026-09-12 (plan §§10–11)**: skips F4–F11 por diseño (F1 no existe en web; F11 como espectador sin errores), timer sin-reloj (`priorityTimeLeftSecs=2147483647` → `isUnlimitedTime()` oculta el badge; PlayerInfoBar + `utils/timer.ts` + 3 tests, suite 1368), match Bo3 `bo3-1` (winsNeeded 2, 3 partidas 2-1, lobby "Wins: 2 Score", pips ○○→●○). **Límites**: ~~`SideboardScreen` solo con tests~~ (plan2 A.3 cerrado: submit por UI en vivo, ver U4/C1); ~~reconexión web mid-game sin verificar~~ (plan2 A.7 cerrado por p6-C4, `docs/history/qa/p6-live-validations.md`). **En vivo 2026-09-18 (p6-C1/C4)**: Bo3 real, `SideboardScreen` con submit por UI y partida 2 correcta (biblioteca 54); recarga mid-game → el proxy reenvía el último estado+prompt («has rejoined the game», `__mageScene.gameView` turno 1/biblioteca 54/exilio 2, gameId correcto) y «Pasar Prioridad» de la UI avanza el prompt del server. **Bug abierto F1 (p6-C1)**: al arrancar la partida 2 el `GAME_INIT` nuevo no refresca el `GameView` (el tablero queda con la partida 1 hasta recargar) — candidato a fix en la transición Bo3 | 2026-09-18 |
| U12 | Zonas y jugador | `game/PlayAreaPanel.java`, `BattlefieldPanel.java`, `HandPanel.java`, `PlayerPanelExt.java`, `cards/*` | `board/*` (37: `BoardZone`, `HandBar`, `StackZone`, `CommandZone`, `Pile`…) | ✅ | Cerrada 2026-09-08 (ver § U12): G12-1 menú clic-derecho por bando + flujo permiso completo + Switch Hands (verify real 19/19), G12-2 disparadores deck/sideboard + InfoWindows auto (e2e), G12-3 `CARD_CONTEXT_ITEMS` eliminado (sin menú de carta en desktop para gameplay), G12-4 filtro `phasedIn` (unit+e2e). Supera en layouts + HandViewer. **En vivo 2026-09-12 (plan §§7–9, §11)**: fichas/auras/contadores/PW/equipos/evasión renderizados (detalle en § U12); visor de cementerio verificado (1 carta, ×/Esc); chat de espectador verificado. Exilio/UserAction-humano/FFA siguen en tests (plan2 A.5/A.6/A.8). **En vivo 2026-09-18 (p6-C2, `docs/history/qa/p6-live-validations.md`)**: visor de **exilio** interactivo verificado — `mage_cheat_setup` exilió Lightning Bolt + Counterspell, chip «Exilio: 2» abre `PileOverlay` «Exilio (2 cartas)» con ambas cartas, hover-preview Scryfall y cierre ×; el clic en carta no jugable es no-op por diseño (sólo `onClick` para jugables/objetivo) | 2026-09-18 |
| U13 | Combate / Maná | `combat/CombatManager.java`, `game/ManaPool.java` | `game/feedbackModes/CombatBar.tsx`, `ManaBar.tsx`, `ResourceBar.tsx` | ✅ | Auditada 2026-09-05: G13-1 cerrado (fix 2026-09-06), resto ✅. **En vivo 2026-09-12 (plan §§7–9)**: evasión flying (Serra 20→16), vigilance (no gira), haste (Greaves), trample (Rancor, icono + badge TR), shroud positivo/negativo (→`ABILITY_HEXPROOF`; sin objetivo legal no se ofrece equip). **Quirk engine declarado (fork, fuera de alcance)**: Ballista en declare-attackers abre `GAME_CHOOSE_ABILITY` en vez de declarar (4 repros, plan §8); la web manda el mismo UUID que el desktop (paridad por construcción) y el ataque normal está cubierto (tokens + `combat.spec` real) | 2026-09-12 |
...
| U15 | Sistema | `dialog/PreferencesDialog.java`, `DownloadImagesDialog.java`, `GameEndDialog.java`, `AddLandDialog.java`, `CardInfoWindowDialog.java`, `AboutDialog.java`, `WhatsNewDialog.java` | `settings/SettingsModal.tsx`, `game/GameEndDialog.tsx`, `game/HelpWikiModal.tsx`, `system/AboutModal.tsx`, `system/news.ts`, `system/gameLogs.ts` | ✅ | Auditada y cerrada 2026-09-08 (ver § U15): prefs esenciales ✅ + atajos fijos (diferencia declarada), imágenes retiradas (descarga de cartas/símbolos oculta 2026-09-10: dependía de Cache Storage, no disponible sobre HTTP), fin partida ✅ + duración + auto-log, tierras ✅ + set picker full-art, InfoWindows ✅ (U12), About ✅ + news remoto doble feed (Nexus+XMage vía GitHub releases, gratis). **En vivo 2026-09-12 (plan §§10–11)**: timer sin-reloj (`isUnlimitedTime`, badge oculto) · visor singular "Cementerio (1 carta)" (`PileOverlay` + test) · diálogo de fin ("Partida finalizada", duración, Descargar log, Volver al lobby) · feed limpio al espectar re-verificado | 2026-09-12 |
...
**En vivo 2026-09-12 (plan §§8–9)**: diálogo X (`GAME_GET_AMOUNT` "Announce the value for
{X}{X}", Ballista X=2 con pago {4}) · `GAME_CHOOSE_ABILITY` (Jace +2/0 con legalidad del
engine; Equip {0} con auto-target a una sola criatura) · `GAME_TARGET` a jugador (Jace +2) +
`GAME_ASK` Yes/No · orden/devolución (Brainstorm 2× put-back secuencial) · ping de Ballista
con coste (`GAME_TARGET` a jugador). **Límite**: diálogo multi-cantidad en web sin verificar
en vivo (el gang-block fue por harness MCP directo a `GAME_GET_MULTI_AMOUNT` sin prompt de
orden; plan §6) — queda en tests.
...
| Permisos de mano: pedir/ver/autorizar/revocar + Switch Hands (Mindslaver) | Cerrado en U12 abajo (menú por bando + comandos + Switch Hands cuando el servidor envía manos). **Control total estilo Mindslaver cerrado 2026-09-17** (actuar por el otro): helper `state/control.ts` (`opponentHands` + `priorityPlayerName`), `canPass`/`ActionButton` habilitados con "Controlas el turno de {name}" (`game.controlling_turn` ×9), Switch Hands por store alimentando la `handBar` del shell (GameBoard/Arena/TwoHeads), zona del controlado interactiva (playables + combate con `handleCardClick`) y guarda anti auto-pass; unit (`control.test.ts`, `ActionButton.control.test.tsx`, `GameBoard.control.test.tsx`, `BoardZone.handSwitch.test.tsx`) + e2e `control.spec.ts` (`@control`, fake, incluye la ventana de combate). **Verificado en vivo 2026-09-17** (MCP+navegador, partida real, web attachada a la sesión MCP): el pase desde el botón avanza el prompt del servidor; el Switch Hands muestra la mano del controlado; el clic en su Isla jugable la gira (`tapped:true` + `manaPool blue:1`). Sin cambios Java (el server ya enruta `sendPlayerBoolean`/`sendPlayerUUID` del controlador) | ✅ G12-1 (ver U12) |
| Visores: looked-at, companion dedicada, top-library, sideboard solo-ver | Cerrado en U12 abajo (`InfoWindows` + disparadores + `LimitedDeckDialog`) | ✅ G12-2 (ver U12) |
| Menú contextual botón derecho sobre jugador | Cerrado en U12 abajo: no existe en desktop para gameplay → `CARD_CONTEXT_ITEMS` eliminado del código; el gesto vive a nivel jugador | ✅ G12-3 (ver U12, veredicto: no-aplica) |
| Filtro `phasedIn` (oculta faseados) | Cerrado en U12 abajo: filtro `p.phasedIn !== false` en `BoardZone.tsx:230` + test | ✅ G12-4 (ver U12) |
| Hover carta grande, flechas targeting/combate, sonidos tablero | `FloatingCardPreview` + `CombatArrowsOverlay` + 15 sfx | ✅ |

Veredicto: sin gaps abiertos (U12 cerrada 2026-09-08; Mindslaver total cerrado en cliente 2026-09-17 — `state/control.ts` + `@control`).
...
| Switch Hands al controlar otro turno (`GamePanel.java:1118`) | Toggle ⟲ en la zona propia cuando `opponentHands` trae manos (`handSwitch.ts` + `BoardZone`, badge con el nombre). **2026-09-17**: el estado vive en el store (`switchedHandKey` + `setSwitchedHandKey`, poda en `eventHandler`) y alimenta la `handBar` del shell (la mano cambiada sustituye a la propia en standard/arena/pod, `switchedHandCards` normaliza `id`+`faceDown:false`); botón montado por portal como hijo del `.board-shell` (la zona crea su propio contexto de apilamiento y ningún `z-index` interno gana al abanico; verificado en vivo 2026-09-17: clicable con la mano llena); click envía el UUID del controlado. Unit `BoardZone.handSwitch.test.tsx`/`GameBoard.control.test.tsx` + e2e `control.spec.ts` | ✅ G12-1 |
...
**En vivo 2026-09-12 (plan §§7–9, §§10–11)**: fichas (`tokens-1`: `Dragon Fodder ORI:140` →
2× Goblin 1/1 `isToken`/`TOKEN`, daño 20→19→17) · auras (`aura-1`: `Elvish Mystic m14:169` +
`Rancor M13:185`, auto-objetivo con una sola criatura legal, 3/1+trample y 5/1 doble,
`attachments[]`/`attachedTo`, icono `ABILITY_TRAMPLE`, badge TR) · contadores
(`counters-1`: `Walking Ballista 2XM:306` X=2, 2/2 con 2×+1/+1, ping con coste T7
20→19, +1 contador T9, `counters` + icono `OTHER_COST_X x=2`, badge `+2` + chip `x=2`) ·
planeswalkers (`pw-1`: `Jace, the Mind Sculptor WWK:31`, lealtad 3→5 con +2, Brainstorm 0
con 2× put-back secuencial) · equipos/evasión (`equip-1`: Greaves T4 + Boots T6 + `Serra
LEA:39` T10, Equip auto-target, Flying/Vigilance/Haste/Shroud — shroud→`ABILITY_HEXPROOF`
en engine —, ataque 20→16 con vigilance sin girar; shroud negativo: sin objetivo legal no
se ofrecen en `playableIds`) · visor de **cementerio** (`audit-2c3`: abre/pinta/cierra
con ×/Esc, singular "(1 carta)", arte M10) · chat de partida como espectador. **Actualizado
2026-09-18 (p6, `docs/history/qa/p6-live-validations.md`)**: visor de **exilio** verificado en vivo
(2 cartas exiliadas con `cheat_setup`, overlay «Exilio (2 cartas)», hover-preview, cierre ×) —
plan2 A.5 cerrado; `UserActionModal` = modal de usuario del lobby (mismo flujo que U5, ver
U5/C3), sin camino de juego distinto — plan2 A.6 cerrado como "mismo flujo";
3+ jugadores FFA sigue cubierto por plan4 §3.11 (pod/commander/ffa-six).

### U3 — Crear mesa (AUDITADA 2026-09-04, CERRADA 2026-09-06)

Ver `../docs/history/lobby_roadmap.md` §F (tablas F1–F10, U1–U8 + delta 2026-09-06). F6 emblemas declarado fuera de alcance.
...
| Join: password siempre visible + recordada | Solo si `passworded`, sin caché; activa también en `beta.xmage.today` (el motor comprueba la password de mesa siempre) | ✅ U4-2 cerrado 2026-09-14 (`JoinTableDialog`); **2026-09-20**: disable en beta revertido para mesas (verificado en vivo con 2 cuentas + UI) |
...
| Espera: polling + sonidos join/leave/start | Broadcast lobby + sonidos join/leave (`player_join`/`player_leave`); start ya sonaba (`game_start`) | ✅ U4-12 cerrado 2026-09-14 (`stagingSounds.ts` + diff de asientos en `eventHandler`, sinte nueva) |
...
### U8 — Generador mazos (fuera de alcance por decisión 2026-09-14)

Sin equivalente web. Declarado fuera de alcance.
...
| Mana Analyser 4 gráficos (pips, básicas, fuentes, distribución) | `CurveChart` + **donuts SVG reales `ManaPie` (sectores, total central, leyenda con nombre/cantidad/%)** + 3 bloques (fuentes tierras/no-tierras, básicas por color, barras apiladas pips×CMC) | ✅ U6-2 (`oracleText` añadido al meta; heurística fuentes documentada en `deckUtils`; donuts reescritos 2026-09-14) |
