...
- **Solved** (Casos MKM): el cliente YA pinta el badge `CaseSolved`
  (`web/src/board/designations.ts:43`; fila rendered en
  `web/src/board/hintRegistry.ts:38`; i18n ×9). En el motor `CaseSolvedHint`
  sigue sin adjuntarse (`CaseSolvedHint.java`; `CaseAbility.java` no lo añade),
  así que depende de upstream: **gap de motor aceptado (2026-09-18), sin parche
  de fork**.
...
- **Resto del baseline, descartado como bookkeeping o cubierto**: `deathtouched`,
  ...
  + `leftHalfUnlocked`/`rightHalfUnlocked` (room de Duskmourn, NO mazmorra;
  `unlockDoor` solo informa por chat — `PermanentImpl.java:2346-2356` — sin campo
  en `PermanentView`; aceptado 2026-09-18, sin parche de fork),
  `loyaltyActivationsAvailable/canBeSacrificed` (el servidor
...
| Speed (Aetherdrift) | ✅ | ✅ | presencia vía `designationNames` (`Speed`; `PlayerView.java:156-158`); badge `PlayerInfoBar.tsx:201-207,452-465` + test `PlayerInfoBar.test.tsx:122-142`; tab `MechanicsTray.tsx:159-161,193,438-452`. **Nivel 1–4 no emitido por upstream** (baseline `engine-view-gap.baseline.json:114`) — aceptado 2026-09-18, sin parche de fork | 2026-09-18 |
...
| Objetivo en la pila (contrahechizo / Stifle) | ✅ | ✅ | `StackZone` marca `targetable` las entradas elegibles (`StackZone.test.tsx`) y `GameScreen` abre el cajón de la pila cuando un objetivo válido está en ella (`stackTargeting.ts` + test); `e2e/stack-target.spec.ts` (escenario `stackTarget.ts`): con otra pestaña abierta llega el `GAME_TARGET`, la pila vuelve sola, cerrar/reabrir el cajón mantiene la entrada seleccionable y el clic llega al servidor con el uuid | 2026-09-24 |
...
| Coste para atacar (Propaganda-like: "pay {2} to attack") | ✅ | ✅ | Bug reportado por el usuario (2026-09-23): al pagar {2} para atacar con Treasure tokens sin girar disponibles, no eran clicables. **Hallazgo**: durante el `GAME_PLAY_MANA` de este coste, `canPlayObjects.objects` llega **vacío** ({}) aunque haya Treasure/tierras sin girar (a diferencia del pago normal de un hechizo, donde sí aparecen); el servidor igualmente acepta `sendPlayerUUID` de cualquier fuente válida como pago. El bug real estaba en el web: `useBoardPresenter.ts` gateaba el click de cualquier permanente en `playableIds` (1:1 con `canPlayObjects`) — con éste vacío, ningún permanente era clicable. El cliente oficial (`Mage.Client` `CardPanel.mouseClicked`) no gatea el click en `isPlayable`, siempre reenvía el UUID. **Fix**: durante un feedback de maná (`feedback.mode === 'mana'`) el web reenvía cualquier click de permanente aunque no esté en `playableIds` (`manaFeedbackActive` en `useBoardPresenter.ts`). El vacío de `canPlayObjects` **no es universal** a todo pago fuera del cast: `fixtures/recorded/echo-upkeep.json` (Echo {3}{G} pagado en el upkeep) muestra `canPlayObjects` **poblado** correctamente para ese sub-loop — el gap está escoped a (al menos) el coste para atacar, no a triggers de upkeep resolviendo; el fix igual cubre ambos casos sin distinguirlos. Frames reales `fixtures/recorded/attack-cost.json` (Propaganda cheateada al SIM; Grizzly ataca pagando {2} con un Treasure sacrificado + una Forest; invariante `hasAttackCostPaid`) y `fixtures/recorded/echo-upkeep.json` (Citanul Centaurs, Echo {3}{G}; invariante `hasEchoUpkeepPaid`); unit `useBoardPresenter.test.ts` (dispatcher de clicks) | 2026-09-23 |
| Smart mana payment (opt-in auto-tap during `GAME_PLAY_MANA`) | ✅ | ✅ | `game/smartManaPayment.ts` re-solves at every prompt (the server asks once per tap): cost from the tag-stripped `prompt.message`, sources from the controlled player's battlefield `rules` (`{T}: Add …`), never from `canPlayObjects` (empty in non-cast payments, see `recorded/attack-cost.json`); exact matching for colored/hybrid/generic pips, then a tie-break that keeps the last producer of a colour needed by an instant/flash card in hand, weighted by the opponent's visible untapped mana. Skips Treasure-style sacrifice sources, restricted mana (Cavern) and convoke; gives up for the rest of the payment when the remaining cost does not drop by exactly one (manual takeover). Units `smartManaPayment.test.ts`, `ManaBar.smart.test.tsx`; real engine `e2e/smart-mana-real.spec.ts` (Boros Charm paid by exactly one Mountain + one Plains, no manual clicks) | 2026-09-23 |
...
| Fallar la búsqueda (fail to find) | ✅ | ✅ | Frame real `recorded/fail-to-find.json` (Evolving Wilds + declinar): `TargetCardInLibrary.setRequired(!filter.hasPredicates())` (regla 701.15b) → `GAME_TARGET` con **`flag:false`**; la UI ya ofrecía "Terminar" con `required===false` (`TargetBar.tsx:40-42`, `CardGrid.tsx:142-148`; `useFeedbackForm.ts:99-101` envía `sendPlayerBoolean(false)`). Cobertura añadida 2026-09-18: `web/e2e/fail-to-find.spec.ts` (@failtofind, escenario `required:false`; declinar no mueve carta y la biblioteca queda intacta) + caso `flag:'false'` en `feedback.test.ts`; invariante `hasFailToFind` | 2026-09-18 |
...
| Ceder el control al rival en SU upkeep (Slicer, Hired Muscle) | ✅ | ✅ | Frame real `fixtures/recorded/slicer.json` (cheatSetup de Slicer al campo propio; en el upkeep del SIM el trigger `BeginningOfUpkeepTriggeredAbility(OPPONENT)` pregunta al CONTROLADOR con `GAME_ASK` — **ojo: el texto llega con `{this}` sin resolver, no matchear por el nombre** — y se responde SÍ): Slicer sale en el battlefield del SIM (`controllerName` del rival), `tapped:false` (untap) y el goad + "can't be sacrificed" viajan en `rules` + cardIcon `OTHER_HAS_RESTRICTIONS` ("Goaded by … (must attack)", mismo camino que `goad.json`; `goadingPlayers` no se serializa); sin campos nuevos del contrato; invariante `hasSlicerCeded` | 2026-09-18 |
