...
import { clearActiveGame, saveFxSettings, saveAudioSettings, saveAppearanceSettings, saveAutoAnswers, saveManaPayment, applyAppearanceToDocument } from './persistence'
...
export function setSetting<K extends keyof AppState['settings']>(key: K, value: AppState['settings'][K]) {
  ...
  const { effects, animationSpeed, soundEnabled, masterVolume, sfxVolume, uiVolume, sleeveId, boardLayout, uiScale, cjkBoost, autoAnswers, manaPayment } = getState().settings
