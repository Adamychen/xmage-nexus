...
export default function SummaryStrip({ form }: { form: CreateTableForm }) {
  ...
  return (
    <div className="create-table-summary-strip">
      ...
      {(form.mulliganType !== 'GAME_DEFAULT' || form.customStartLifeEnabled || form.customStartHandSizeEnabled || form.planeChase) && <span className="summary-pill" style={{ borderColor: 'rgba(124,92,255,0.4)' }}>🎲 Custom ({[form.mulliganType !== 'GAME_DEFAULT' ? form.mulliganType : null, form.customStartLifeEnabled ? `Vida ${form.customStartLife}` : null, form.customStartHandSizeEnabled ? `Mano ${form.customStartHandSize}` : null, form.planeChase ? 'Planechase' : null].filter(Boolean).join(' · ')})</span>}
      {form.compatibilityError && <span className="summary-pill security">⚠️ {form.compatibilityError.slice(0, 28)}</span>}
      {form.isDraftLimited && <span className="summary-pill">🃏 Draft {form.draftBoosters}× {parseLimitedSetCodes(form.draftSetsRaw).join(', ') || 'sets'}</span>}
      {form.isLimited && !form.isDraftLimited && <span className="summary-pill">{t('lobby','create_summary_limited')}</span>}
      {form.minimumRating > 0 && <span className="summary-pill">⭐ {t('lobby','create_field_min_rating')}: {form.minimumRating}</span>}
      {form.quitRatio < 100 && <span className="summary-pill">🚫 {t('lobby','create_field_quit_ratio')}: {form.quitRatio}%</span>}
      {form.password.trim() && <span className="summary-pill security">🔒 {t('lobby','tag_private')}</span>}
      {form.rated && <span className="summary-pill rated">⭐ {t('lobby','tag_rated')}</span>}
    </div>
  )
}
